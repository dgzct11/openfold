...
def run_mmseqs2(x, prefix, use_env=True, use_filter=True,
                ...
                user_agent: str = "colabdesign/gamma") -> Tuple[List[str], List[str]]:
  ...
  if not os.path.isfile(tar_gz_file):
    ...
    with tqdm(total=TIME_ESTIMATE, bar_format=TQDM_BAR_FORMAT) as pbar:
      while REDO:
        ...
        while out["status"] in ["UNKNOWN", "RATELIMIT", "ERROR"]:
          if out["status"] == "ERROR" and "msg" in out:
            print("Sleeping for 30 minutes due to error:", out["msg"])
            time.sleep(1800)
    ...
    print(f"Skipping mmseqs2 search as {tar_gz_file} already exists.")
...
# options
fasta_dir = "/home/zahadg/projects/AlphafoldSAE/fasta_dir"
output_dir = "/home/zahadg/projects/AlphafoldSAE/output_dir"

# read fasta files
seqs = []
for fasta in os.listdir(fasta_dir):

  #check if it exists in the output_dir
  if os.path.exists(f"{output_dir}/{fasta.split('.')[0]}"):
    print(f"Skipping {fasta} as it already exists")
    continue
  if fasta.endswith(".fasta"):
    with open(f"{fasta_dir}/{fasta}", "r") as f:
      lines = f.readlines()
      seqs.append((
        lines[0].rstrip()[1:],
        lines[1].rstrip()
...
print(seqs[:4])
with tqdm(total=len(seqs), desc="Processing sequences", bar_format=TQDM_BAR_FORMAT) as pbar:
  for seq in seqs:
    run_mmseqs2(seq[1], prefix=f"{output_dir}/{seq[0]}", use_templates=False, use_env=False)
    pbar.update(1)
